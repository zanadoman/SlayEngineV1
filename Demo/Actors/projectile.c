...
uint8 updateProjectile(slayEngine* Engine)
{
    ...
    switch (Engine->CurrentScene)
    {
        ...
        case 1:
            ...
            Projectiles = ((scene1*)Engine->Scenes->Values[1])->Projectiles;
            Platforms = ((scene1*)Engine->Scenes->Values[1])->Platforms;
            Crate = ((scene1*)Engine->Scenes->Values[1])->Crate;
            SoundFire = ((game*)Engine->Game)->Sounds->gunFire;
        break;
    ...
    for (uint16 i = 0; i < Projectiles->Length; i++)
    {
        ...
        for (uint16 j = 0; j < Platforms->Length; j++)
        {
            ...
            if (slayCollNONE < collision || (((projectile*)Projectiles->Values[i])->X < ((projectile*)Projectiles->Values[i])->MinX - ((projectile*)Projectiles->Values[i])->Width || ((projectile*)Projectiles->Values[i])->MaxX < ((projectile*)Projectiles->Values[i])->X) || (((projectile*)Projectiles->Values[i])->Y < ((projectile*)Projectiles->Values[i])->MinY - ((projectile*)Projectiles->Values[i])->Height || ((projectile*)Projectiles->Values[i])->MaxY < ((projectile*)Projectiles->Values[i])->Y))
            {
...
uint8 playerProjectile(slayEngine* Engine, array Projectiles, player* Player, slaySound* SoundFire, uint8 Volume)
{
    ...
    if (slayKey(Engine, Player->KeyFire) && Player->ReloadTime < slayGetTicks() - Player->ReloadTick)
    {
        ...
        if ((Player->Facing == -1 && 90 < angle && angle < 270) || (Player->Facing == 1 && (270 < angle || angle < 90)))
        {
            ...
            arrInsert(Projectiles, Projectiles->Length, newProjectile(actPLAYER, Player->X + Player->CenterX, Player->Y + Player->CenterY, Player->MinX, Player->MinY, Player->MaxX, Player->MaxY, angle));
...
uint8 eagleProjectile(slayEngine* Engine, array Projectiles, eagle* Eagle, player* Player, array Platforms, crate* Crate, slaySound* SoundFire, uint8 Volume)
{
    ...
    if (length <= Eagle->AttackRange && Eagle->ReloadTime < slayGetTicks() - Eagle->ReloadTick && ((Eagle->Facing == -1 && 90 < angle && angle < 270) || (Eagle->Facing == 1 && (270 < angle || angle < 90))))
    {
        ...
        arrInsert(Projectiles, Projectiles->Length, newProjectile(actEAGLE, Eagle->X + Eagle->Width / 2.0, Eagle->Y + Eagle->Height / 2.0, Eagle->MinX, Eagle->MinY, Eagle->MaxX, Eagle->MaxY, angle));
        if (Player->X < Eagle->X)
        {
